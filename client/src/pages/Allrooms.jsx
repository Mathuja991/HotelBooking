          ...
          x1:px-32'>
            <div>
              ...
              {roomsDummyData.map((room)=>(
                ...
                <img onClick={()=>{navigate(`/halls/${room._id}`);scrolledTo(0,0)}}
                src={room.images[0]} alt="room-img"  title='View Hall Details'
                className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer'/>
                
                <div className='md:w-1/2 flex flex-col gap-2'>
                    <p className='text-gray-500'> {room.hotel.city}</p>
                    <p onClick={()=>{navigate(`/halls/${room._id}`);scrolledTo(0,0)}} className='text-gray-800 text-3xl font-playfair cursor-pointer'> {room.hotel.name}</p>
                   <div className='flex items-center'>
                    <StarRating />
                    <p className='mi-2'>200+ reiews</p>
                   </div>
                    ...
                    <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                        {room.amenities.map((item,index)=>(
                            <div key={index} className='flex flex-wrap items-center mt-3 mb-6 gap-4 '>
