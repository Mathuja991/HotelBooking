...
const HallCard = ({room,index}) => {
  return (
    <Link  className='relative max-w-70 w-full rounded -xl overflow-hidden
      bg-white text-gray-500/90 shadow-[0px_4px_4px_rgba(0,0,0,0.05)' to={'/halls' + room._id} onClick={()=> scrollTo(0,0)} key={room._id} >
     <img src={room.images[0] } alt =""  />
     
     { <p className='px-3 py-1 absolute top-3 left-3 text-xs bg-white
      text-gray-800 font-medium rounded-full'>Best choice</p>}
